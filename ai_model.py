...
#     """Strict JSON extraction from API response"""
#     try:
...
#     try:
#         # Try to extract just the inner JSON part
#         start = content.find('{')
#         end = content.rfind('}')
...
#             {"role": "user", "content": prompt}
#         ],
#         "temperature": max(0, min(settings.ai_temperature, 2)),  # Clamp between 0-2
#         "max_tokens": min(4000, settings.ai_max_tokens)  # Prevent exceeding limits
#     }
...
# # def _process_ai_response(response) -> str:
# #     """Handle both streaming and non-streaming with better error checks"""
# #     try:
# #         if settings.ai_stream:
# #             full_content = ""
# #             for line in response.iter_lines():
...
#     if not settings.ai_model_name:
#         raise ValueError("AI model name not configured")
#     if not isinstance(settings.ai_max_tokens, int) or settings.ai_max_tokens < 100:
#         logger.warning(f"Invalid max_tokens value: {settings.ai_max_tokens}, using default 2000")
...
#             {"role": "user", "content": prompt}
#         ],
#         "temperature": max(0, min(float(settings.ai_temperature), 2)),  # Double validation
#         "max_tokens": min(4000, int(settings.ai_max_tokens))
#     }
